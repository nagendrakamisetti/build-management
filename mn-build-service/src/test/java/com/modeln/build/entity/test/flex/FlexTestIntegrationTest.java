...
public class FlexTestIntegrationTest extends AbstractEntityIntegrationTest {
	...
	public void testSimpleCRUDOperationOnLegacyTable() throws Exception {		
		...
		//Count number of Entities
		Query queryCountFlexTests = em_bc.createQuery ("SELECT count(x) FROM FlexTest x");
		Number countFlexTests = (Number) queryCountFlexTests.getSingleResult ();
		assertTrue(!countFlexTests.equals(0));
		
		//Retrieve Entities
		Query queryFindAllFlexTest = em_bc.createQuery("SELECT g FROM FlexTest g");
		queryFindAllFlexTest.setMaxResults(10);
		List<FlexTest> flexTests = queryFindAllFlexTest.getResultList();
		assertTrue(flexTests.size() != 0);
...
//		flexTest.setGroupName("groupName");
		...
		flexTest = em_bc.find(FlexTest.class, flexTest.getId());
...
//		assertTrue(flexTest.getGroupName().equals("updateGroupName"));
		...
		Query queryCountFlexTestAfter = em_bc.createQuery ("SELECT count(x) FROM FlexTest x");
		Number countFlexTestAfter = (Number) queryCountFlexTestAfter.getSingleResult ();
		assertTrue(countFlexTests.intValue() == countFlexTestAfter.intValue());
	}
}
