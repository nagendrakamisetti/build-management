...
public class ActTestCaseMapIntegrationTest extends AbstractEntityIntegrationTest {
	...
	public void testSimpleCRUDOperationOnLegacyTable() throws Exception {		
		...
		//Count number of Entities
		Query queryCountActTestCaseMaps = em_bc.createQuery ("SELECT count(x) FROM ActTestCaseMap x");
		Number countActTestCaseMaps = (Number) queryCountActTestCaseMaps.getSingleResult ();
		assertTrue(!countActTestCaseMaps.equals(0));
		
		//Retrieve Entities
		Query queryFindAllActTestCaseMap = em_bc.createQuery("SELECT g FROM ActTestCaseMap g");
		queryFindAllActTestCaseMap.setMaxResults(10);
		List<ActTestCaseMap> actTestCaseMaps = queryFindAllActTestCaseMap.getResultList();
		assertTrue(actTestCaseMaps.size() != 0);
		...
		actTestCaseMap = em_bc.find(ActTestCaseMap.class, new ActTestCaseMapId(act.getId(), actTestCaseMap.getTestcase()));
		...
		Query queryCountActTestCaseMapAfter = em_bc.createQuery ("SELECT count(x) FROM ActTestCaseMap x");
		Number countActTestCaseMapAfter = (Number) queryCountActTestCaseMapAfter.getSingleResult ();
		assertTrue(countActTestCaseMaps.intValue() == countActTestCaseMapAfter.intValue());
	}
}
