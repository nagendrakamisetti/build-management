...
public class Add {
	...
	/**
	 * Open existing file(s) for add.  The file(s) will be opened as its previous 'filetype' and
     ...
     * @return			return a list of files opened for add; return null if action failed
	 */
	public static boolean openFile (String file) {
		if ((file==null)||(file=="")) {
			throw new IllegalArgumentException("File name not supplied.");
		}
		String command = "p4 add "+file;
	...
	public static boolean openFile (String file, String changelistNumber) {
		...
		String command = "p4 add "+file+" -c "+changelistNumber;
		return execute(command);
	}
	
	/**
	 * Open existing file(s) for add.  The file(s) will be opened as 'filetype' and
	 * will be put into a pending changelist.
     * <p>
     * <blockquote>
	...
	public static boolean openFile (String file, String changelistNumber, String fileType) {
		...
		if ((fileType==FILETYPE_TEXT)||(fileType==FILETYPE_BINARY)||(fileType==FILETYPE_SYMLINK)
			...
			String command = "p4 add "+file+" -c "+changelistNumber+" -t "+fileType;
			return execute(command);
		} else {
