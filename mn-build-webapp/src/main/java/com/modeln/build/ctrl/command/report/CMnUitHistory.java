 ...
 */
public class CMnUitHistory extends TrustedHostCommand {
        ...
        throws ApplicationException
    {
        ...
        if (!result.containsError()) {
            ...
            try {
                ...
                CMnDbUit testData = uitTable.getTest(rc.getConnection(), testId); 
                ...
                if (testData.getStartTime() != null) {
                    CMnDbUit lastPass = uitTable.getLastPass(rc.getConnection(), testData);
                    req.setAttribute(IMnTestForm.LASTPASS_OBJECT_LABEL, lastPass);

                    CMnDbUit lastFail = uitTable.getLastFailure(rc.getConnection(), testData);
                    req.setAttribute(IMnTestForm.LASTFAIL_OBJECT_LABEL, lastFail);
                }

                // obtain the test history data from the database
                Vector uitHistory = uitTable.getTestHistory(rc.getConnection(), testData);
                //TODO: testing...
                if(uitHistory==null) {
                	uitHistory.add(testData);
                }
                req.setAttribute(IMnTestForm.HISTORY_OBJECT_LABEL, uitHistory);
                ...
                CMnDbTestSuite suiteData = uitTable.getSuite(rc.getConnection(), suiteId);
                if (suiteData != null) {
                ...
                result.setDestination("reports/uit_history.jsp");
