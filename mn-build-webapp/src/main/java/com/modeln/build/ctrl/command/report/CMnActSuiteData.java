...
/**
 * The ACT suite command displays information for a single
 * ACT suite. 
 * 
 * @author             Shawn Stafford
 */
public class CMnActSuiteData extends TrustedHostCommand {
        ...
        if (!result.containsError()) {
            ...
            try {
                ...
                if ((suiteId != null) && (suiteId.length() > 0)) {
                    suiteData = actTable.getSuite(rc.getConnection(), suiteId);
                    testList = actTable.getTestsBySuite(rc.getConnection(), suiteId);
                    if (suiteData != null) {
                    ...
                    testList = actTable.getTestsBySuiteGroup(rc.getConnection(), groupId);

                    // Attempt to retrieve the build ID indirectly by following the test/suite relationship
                    if ((testList != null) && (testList.size() > 0)) {
                        CMnDbAcceptanceTestData test = (CMnDbAcceptanceTestData) testList.get(0);
                        if ((test != null) && (test.getParentId() > 0)) {
                            suiteData = actTable.getSuite(rc.getConnection(), Integer.toString(test.getParentId()));
                    ...
                    if ((buildIdStr != null) && (buildIdStr.length() > 0)) {
                        ...
                        testList = actTable.getTestsByGroupName(rc.getConnection(), buildIdStr, groupName);
                    }
                ...
                if (testList != null) {
                    req.setAttribute("ACT_TEST_LIST", testList);
                }


                result.setDestination("reports/act_suite.jsp");
