...
/**
 * The test history command returns the results of every test execution
 * that is comparable to the specified test ID.  A comparable test
 * is one that was conducted using the same build, JVM, OS, and hardware. 
 * 
 * @author             Shawn Stafford
 */
public class CMnActTestHistory extends TrustedHostCommand {
        ...
        throws ApplicationException
    {
        ...
        if (!result.containsError()) {
            ...
            try {
                ...
                CMnDbAcceptanceTestData testData = actTable.getTest(rc.getConnection(), testId); 
                ...
                if (testData.getStartTime() != null) {
                    CMnDbAcceptanceTestData lastFail = actTable.getLastFailure(rc.getConnection(), testData);
                    req.setAttribute(IMnTestForm.LASTFAIL_OBJECT_LABEL, lastFail);
                
                    CMnDbAcceptanceTestData lastPass = actTable.getLastPass(rc.getConnection(), testData);
                    req.setAttribute(IMnTestForm.LASTPASS_OBJECT_LABEL, lastPass);
                }

                // Obtain the historical test results 
                Vector tests = actTable.getTestHistory(rc.getConnection(), testData);
                req.setAttribute(IMnTestForm.HISTORY_OBJECT_LABEL, tests);
                ...
                CMnDbTestSuite suiteData = actTable.getSuite(rc.getConnection(), suiteId);
                if (suiteData != null) {
                ...
                result.setDestination("reports/act_history.jsp");
